...
│                           MCP Server 层                                      │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐            │
│  │ query_knowledge  │  │ list_collections│  │ get_document    │            │
│  │ _hub             │  │                 │  │ _summary        │            │
│  └─────────────────┘  └─────────────────┘  └─────────────────┘            │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
...
│                         可插拔组件层 (libs)                                  │
│  ┌─────────┐  ┌───────────┐  ┌──────────┐  ┌───────────┐  ┌───────────┐  │
│  │   LLM   │  │ Embedding │  │VectorStore│  │ Reranker  │  │ Evaluator │  │
│  │ Factory │  │  Factory  │  │  Factory  │  │  Factory  │  │  Factory  │  │
│  └─────────┘  └───────────┘  └──────────┘  └───────────┘  └───────────┘  │
│       │            │              │             │              │           │
│       ▼            ▼              ▼             ▼              ▼           │
│  ┌─────────┐  ┌───────────┐  ┌──────────┐  ┌───────────┐  ┌───────────┐  │
│  │OpenAI   │  │OpenAI     │  │ ChromaDB │  │Cross-Enc  │  │ Ragas     │  │
│  │Azure    │  │Azure      │  │          │  │LLM Rerank │  │ Custom    │  │
│  │Ollama   │  │Ollama     │  │          │  │           │  │           │  │
│  │DeepSeek │  │           │  │          │  │           │  │           │  │
│  └─────────┘  └───────────┘  └──────────┘  └───────────┘  └───────────┘  │
└─────────────────────────────────────────────────────────────────────────────┘
...
PDF文件
  ...
  ▼
┌─────────────────────┐
│  FileIntegrity      │  SHA256 去重检查
│  (文件完整性校验)   │
└─────────────────────┘
  │
  ▼
┌─────────────────────┐
│  PdfLoader          │  PDF 解析 + 图片提取
│  (文档加载器)       │
└─────────────────────┘
  │
  ▼
┌─────────────────────┐
│  DocumentChunker    │  语义分块 (RecursiveCharacterTextSplitter)
│  (文档分块器)       │
└─────────────────────┘
  │
  ▼
┌─────────────────────┐
│  ChunkRefiner       │  LLM/规则混合精炼
│  (块精炼器)         │  合并被物理切断的语义段落
└─────────────────────┘
  │
  ▼
┌─────────────────────┐
│  MetadataEnricher   │  LLM/规则混合生成
...
└─────────────────────┘
  ...
  ▼
┌─────────────────────┐
│  BatchProcessor     │  协调 Dense+Sparse 双路编码
│  (批处理器)         │
└─────────────────────┘
  │
  ├──▶┌─────────────────────┐
  │   │  VectorUpserter     │  ChromaDB 向量存储
  │   │  (向量写入器)       │
  │   └─────────────────────┘
  │
  ├──▶┌─────────────────────┐
  │   │  BM25Indexer        │  BM25 倒排索引
  │   │  (稀疏索引器)       │
  │   └─────────────────────┘
  │
  └──▶┌─────────────────────┐
      │  ImageStorage       │  图片索引 (SQLite)
      │  (图片存储器)       │
      └─────────────────────┘
```
...
└─────────────────────┘
  │
  ├──▶┌─────────────────────┐
  │   │  DenseRetriever     │  向量语义检索
  │   │  (稠密检索器)       │  Embedding 相似度匹配
  │   └─────────────────────┘
  │
  ├──▶┌─────────────────────┐
  ...
  └──▶┌─────────────────────┐
      ...
      └─────────────────────┘
              ...
              ▼
┌─────────────────────┐
│  Reranker           │  可选: CrossEncoder / LLM
│  (重排序器)         │  精排提升 Top-K 精准度
└─────────────────────┘
              │
              ▼
┌─────────────────────┐
│  ResponseBuilder    │  组装最终响应 + 引用标注
│  (响应构建器)       │
└─────────────────────┘
```
...
| 设计模式 | 应用场景 | 说明 |
|----------|----------|------|
| **工厂模式 (Factory)** | LLM/Embedding/Splitter/VectorStore/Reranker/Evaluator | 每个可插拔组件均通过工厂创建，支持运行时切换 |
| **抽象接口 (ABC)** | BaseLLM, BaseEmbedding, BaseVectorStore, BaseSplitter, BaseReranker, BaseEvaluator, BaseLoader | 统一接口定义，实现组件解耦 |
| **配置驱动 (Config-Driven)** | `config/settings.yaml` | 一处配置，全链路生效，零代码切换 Provider |
| **流水线模式 (Pipeline)** | IngestionPipeline (6阶段) + HybridSearch (多阶段检索) | 阶段化处理，便于追踪和调试 |
| **观察者模式 (Observer)** | TraceContext + on_progress 回调 | 全链路追踪，实时通知进度 |
| **降级回退 (Graceful Fallback)** | Reranker 失败回退到 Fusion 排序 | 保证系统稳定性，提升容错能力 |
| **幂等设计 (Idempotent)** | SHA256 文件去重 + chunk_id 确定性哈希 + Upsert | 支持重复处理，保证数据一致性 |
...
```
my-rag-server/
├── .opencode/                  # OpenCode Agent Skills 配置
│   └── skills/
│       ├── auto-coder/         # 自动编码 Skill
│       ├── qa-tester/          # 自动测试 Skill
│       ├── resume-writer/      # 简历撰写 Skill
│       ├── interview-prep/     # 模拟面试 Skill
│       ├── project-learner/    # 项目学习 Skill
│       ├── project-review/     # 项目复习 Skill
│       ├── package/            # 清理打包 Skill
│       ├── setup/              # 一键配置 Skill
│       └── skill-creator/      # Skill 创建器
├── config/                     # 配置目录
│   ├── settings.yaml           # 核心运行配置 (8大配置模块)
...
├── src/                        # 核心源代码
│   ├── core/                   # 核心层
│   │   ├── settings.py         # 配置加载与验证 (9个Settings dataclass)
│   │   ├── types.py            # 全链路数据契约
│   │   ├── query_engine/       # 检索引擎核心
...
│   │   │   └── reranker.py             # 重排编排
│   │   ├── response/           # 响应构建层
│   │   │   ├── response_builder.py     # 响应组装
│   │   │   ├── citation_generator.py   # 引用生成
│   │   │   └── multimodal_assembler.py # 多模态组装
│   │   └── trace/              # 全链路追踪
...
│   │   ├── vector_store/       # VectorStore 模块
│   │   ├── splitter/           # Splitter 模块
│   │   ├── reranker/           # Reranker 模块
│   │   ├── evaluator/          # Evaluator 模块
│   │   └── loader/             # Loader 模块
...
├── tests/                      # 三层测试体系
│   ├── unit/                   # 单元测试 (50个文件)
│   ├── integration/            # 集成测试 (13个文件)
│   └── e2e/                    # 端到端测试 (5个文件)
├── DEV_SPEC.md                 # 开发规格文档 (驱动开发的核心设计文档)
├── main.py                     # 项目主入口
...
| **构建系统** | Hatchling | - | 现代化 Python 包构建工具 |
| **包管理** | pip | - | 依赖管理 |
...
| 模块 | 技术选型 | 用途 |
|------|----------|------|
| **LLM 框架** | LangChain | LLM 调用、文本处理 |
| **向量数据库** | ChromaDB | 轻量级向量存储 |
| **MCP SDK** | mcp | Model Context Protocol 服务端实现 |
| **中文分词** | jieba | BM25 稀疏检索的分词处理 |
| **文档解析** | markitdown[pdf] | PDF 文档解析 |
| **评估框架** | Ragas | RAG 质量评估 |
| **可视化** | Streamlit | 管理面板 UI |

### 开发依赖

| 类别 | 技术选型 | 用途 |
|------|----------|------|
| **测试框架** | pytest | 单元/集成/端到端测试 |
| **测试插件** | pytest-cov, pytest-asyncio, pytest-mock | 覆盖率、异步、Mock 支持 |
| **代码风格** | ruff | 代码格式化与 Lint |
| **类型检查** | mypy | 静态类型检查 |
| **LLM 客户端** | openai | OpenAI API 调用 |

### AI Provider 支持

| Provider | LLM | Embedding | Vision | 状态 |
|----------|-----|-----------|--------|------|
| **Azure OpenAI** | ✅ | ✅ | ✅ | 完整支持 |
| **OpenAI** | ✅ | ✅ | ✅ | 完整支持 |
| **Ollama** | ✅ | ✅ | - | 完整支持 |
...
| 工具 | 集成方式 | 状态 |
|------|----------|------|
| **GitHub Copilot** | MCP 配置文件 | ✅ 已集成 |
| **Cursor** | 自动识别 | ✅ 已集成 |
| **Claude Desktop** | MCP 配置文件 | ✅ 支持 |
| **Claude Code** | MCP 配置文件 | ✅ 支持 |
